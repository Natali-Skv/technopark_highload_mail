...
* **хранилище писем пользователя**  
  ...
  Средний объем письма — **75 Кб** [[2]](#источник_2).  
  Пусть **40%** писем содержат вложения, а **60%** не содержат.  
  Пусть также средний размер письма без вложения **6Кб**.  
  Тогда средний размер письма с вложением **178,5 Кб**, где **175 Кб** занимает вложение, а **3,5 Кб** — текст письма.  
...
|-----------------------------------------------|-------------------|
| загрузка главной страницы (входящие)          | 8                 |
| чтение письма                                 | 20                |
| отправка письма                               | 12                |
| просмотр входящих писем (не первая страница)  | 1                 |
| скачивание вложения                           | 6                 |
| удаление письма                               | 3                 |
...
* **RPS в разбивке по типам запросов (запросов в секунду) - для основных запросов**  
  ...
  |----------------------------------------------|-----------------------------------|-----------|-----------|
  | загрузка главной страницы (входящие)         | 8                                 | 1112      | 556       |
  | чтение письма                                | 20                                | 2778      | 1389      |
  | отправка письма                              | 12                                | 1667      | 833       |
  | просмотр входящих писем (не первая страница) | 1                                 | 139       | 69        |
  | скачивание вложения                          | 6                                 | 833       | 417       |
  | удаление письма                              | 3                                 | 417       | 208       |
  | просмотр исходящих писем                     | 2                                 | 278       | 139       |
...
* **Рассчет сетевого потребления на статику.**  
  ...
  Отразим в таблице трафик, переданный по сети (transferred over network).  
...
* **Пиковое потребление в теченнии суток (в Гбит/с). Разбивка по существенным типам трафика (для выбора сетевых
  ...
  |---------------------------------------------|-----------|--------------------------------------|--------------------------|
  | загрузка главной страницы (входящие)        | 1112      |3,4 Мб| 29.5 |
  | чтение письма                               | 2778      |105 Кб| 2.2  |
  | отправка письма                             | 1667      |75 Кб | 1.0  |
  | просмотр входящих писем (не первая страница)| 139       |188 Кб| 0.2  |
  | скачивание вложения                         | 833       |175 Кб| 1.1  |
  | просмотр исходящих писем                    | 278       |188 Кб| 0.4  |


* **Суммарный суточный (Гбайт/сутки) - разбивка по существенным типам трафика (опционально, для подсчета стоимости
  хостинга)**  

  | действие                                    |суточное потребление, Гайт/сутки|
  |---------------------------------------------|--------------------------------|
  | загрузка главной страницы (входящие)        | 159 300                        |
  | чтение письма                               | 11 880                         |
  | отправка письма                             | 5 400                          |
  | просмотр входящих писем (не первая страница)| 1 080                          |
  | скачивание вложения                         | 5 940                          |
  | просмотр исходящих писем                    | 2 160                          |
  | **ИТОГО**                                   | **185 760 Гбайт/сутки**        |
...
- WIDE_MAILS таблица создана, чтобы не дублировать текст письма для каждого из получателей (их может быть несколько) и отправителя. В эту таблицу мы должны ходить только для получения 1 конкретного письма пользователя. С этой целью в таблицы USER_X_DIRECTORY_X_MAILS были добавлены поля Preview, date, theme_preview, has_attaches. 
  - поле USER_REFERENCES []INT -- список айдишников, пользователей, имеющих это письмо. Добавлено для того, чтобы когда пользователь нажимал "скачать вложение" по ID вложения мы определяли соответствующий WIDE_MAIL и проверяли, есть ли текущий пользователь в USER_REFERENCES, то есть может ли этот пользователь скачать это вложение 
...
## 4.<a name="физическая_схема">Физическая схема</a>
  ...
  | Таблица  |Технология| Мотивация | Решающие свойства |
  |----------|--------------|--------|----------|
  | SESSIONS |Tarantool| Очень нагруженная таблица, в нее мы будем ходить почти на каждый запрос пользователя, чтобы проверить ключ сессии. |хранение данных в RAM, cогласованность и сохранность данных в случае сбоя [[6]](#источник_6)|
  | USERS | PostgreSQL |Данные хорошо ложатся на реляционную модель, обновления/вставки крайне редки,  запросы|популярная технология, высокая поддержка.|   
  | ATTACHES | |||   
  | USERS | |||   
  | USERS | |||   
  | USERS | |||   
...
### Таблица SESSIONS будет размещаться в Tarantool.
  В эту таблицу мы будем ходить почти на каждый запрос пользователя, чтобы проверить ключ сессии. 
  Решающие свойства:
    - хранение данных в RAM
    - согласованность и сохранность данных в случае сбоя 
### Таб
