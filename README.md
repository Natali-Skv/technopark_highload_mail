...
|данные            |количество записей      | количество памяти        |пик writes/s |пик reades/s|
|------------------|------------------------|--------------------------|-------|-------|
...
* writes/s -- `rps авторизации + rps регистрации + rps на выход` из *таблицы 4*
* reades/s -- `сумма rps запросов требующих проверку сессии` (все запросы из *таблицы 4*, кроме авторизации и регистрации)
</details>
...
* writes/s -- `rps на редактирование профиля + rps на отправление письма с вложением + rps на удаление вложения` из *таблицы 4*. При запросе на отправку письма с вложением необходимо увеличить user storage_size
* reades/s -- `rps авторизации + rps регистрации` из *таблицы 4*
</details>
...
* writes/s -- `rps на отправление письма` из *таблицы 4*
* reades/s -- `rps на чтение письма` из *таблицы 4*
</details>
...
* writes/s -- `rps на отправление письма с вложением + rps на удаление вложения = 278*40% + 7 = 114` из *таблицы 4*
* reades/s -- `rps на скачивание вложения = 417` из *таблицы 4*
</details>
...
* writes/s -- `rps на регистрацию + rps на редактирование профиля`
* reades/s -- `(rps на загрузку главной страницы + rps на входящие + rps на исходящие + rps на просмотр спама + rps на просмотр корзины)*30*50%`. 30 -- количество писем на странице. Пусть 50% аватарок берется из кеша.
</details>
...
|C++|файловое хранилище, сервис раздачи вложений|высокая производительность|
...
Наибольший вклад в RPS и трафик вносят запросы на чтение письма (2 778*2 RPS, 2,3 Гбит/с). Это тяжеловесные, затратные по времени и памяти запросы, требующие процессинга тела письма для проверки на вирусы, XSS итд. 
На этом наборе бекенд-серверов на одно ядро процессора примем `RPS = 5`  
Получим:
`4 330 RPS/5 RPS = 866 ядер = 40 ядер * 22 серверов`
 </details>
...
**суммарное rps(пик):** `424`    
**суммарный исходящий трафик(пик) в сторону балансировщика:** `10 Мб*417 rps = 33 Гбит/с`
**входящий трафик(пик) от хранилища вложений: `33 Гбит/с`    
...
**суммарное rps(пик):** `278`      
**суммарный входящий трафик(пик):** `9,1 Гбит/с`    
**суммарный исходящий трафик(пик) в БД:** `10 Гбит/с`
