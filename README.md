...
|------------------|------------------------|------------------------|-------|-------|
|SESSIONS          |3 * 20 млн = 60 млн     |164 Б * 60 млн = 9,8 Гб |7,002  |4948   |
|USERS             |20 млн                  |478 Б * 20 млн = 9,6 Гб |119    |7      |
|WIDE_MAILS        |20 млн * 1 250 = 25 млд |100 Кб * 25 млд = 2 Пб  |278    |2778   |
|ATTACHMENTS       |500 * 20 млн = 48 млн   |164 Б * 48 млн = 10 Гб  |114    |417    |
...
USER_X_* набор таблиц -- USER_X_MAILS, USER_X_DIRECTORIES, USER_X_DIRECTORY_X_MAILS, USER_X_ATTACHES. Это персональный набор таблиц для каждого пользователя.
...
|таблица          |количество записей      | количество памяти                             |пик input/day|пик output/day|
|-----------------|------------------------|-----------------------------------------------|-------|-------|
...
|набор таблиц            |количество записей      | количество памяти                             |пик input/day|пик output/day|
|------------------------|------------------------|-----------------------------------------------|-------|-------|
...
* Размер хранилища среднего пользователя 5Гб, то есть `5 Гб/10 Мб = 500 вложений`   
* ipd -- `rps на отправление письма с вложением + rps на удаление вложения = 278*40% + 7 = 114` из *таблицы 4*
* opd -- `rps на скачивание вложения = 417` из *таблицы 4*
  
|таблица            |количество записей  | количество памяти                             |пик input/day|пик output/day|
|-------------------|--------------------|-----------------------------------------------|-------|-------|
|USER_X_ATTACHMENTS | 500                |500 * 90 Б = 0,05 Мб | ||

  
  
таблица на одного пользователя -- расчет размера одного sqlite файла
|таблица          |количество записей      | количество памяти                             |пик ips|пик ops|
|-----------------|------------------------|-----------------------------------------------|-------|-------|
|USER_X_*         |                        |(размер одного sqlite-файла) * 20 млн = __  Гб |7,002  |4948   |
