...
<!-- TODO описать методология рассчета -->
<!-- тут непонятно, как хранить storage -- это по идее путь к индивидуальному хранилищу пользователя. Это поле можно убрать и хранить sqlite-файлы с названиями по id пользователя -->
## **SESSIONS**
|столбец|тип|объем|
|-------|---|-----|
|id|INT|4 Б|
...
|timezone|TIMEZONE|16 Б|
|**Итого**||**172 Б**|

* Пусть каждый пользователь залогинен на 3-х девайсах.
* ips -- `сумма rps запросов требующих проверку сессии` (все запросы из *таблицы 4*, кроме авторизации и регистрации)
* ops -- `rps авторизации + rps регистрации` из *таблицы 4*

|таблица           |количество записей| количество памяти|пик ips|пик ops|
|------------------|------------------|------|---|---|
|SESSIONS          |3 * 16 млн = 48 млн |172 Б*48 млн = 8,3 Гб |4948|7,002|
...
|таблица           |количество записей| количество памяти|пик ips|пик ops|
|------------------|------------------|------|---|---|
|USERS             |20 млн | 490 Б * 20 млн = 9,8 Гб | 119 | 7 |
...
* ips -- `rps на отправление письма` 
* ops -- `rps на чтение письма` 
|таблица        |количество записей              | количество памяти       |пик ips|пик ops|
|---------------|--------------------------------|-------------------------|-------|-------|
|WIDE_MAILS     |20 млн * 1 250 = 25 000 000 000 | 100 Кб * 25 млд = 2 Пб  | 278   | 2778  |
...
|таблица     |количество записей              | количество памяти       |пик ips|пик ops|
|------------|--------------------------------|-------------------------|-------|-------|
|ATTACHMENTS |20 млн * 1 250 = 25 000 000 000 | 100 Кб * 25 млд = 2 Пб  | 278   | 2778  |
