...
* **RPS в разбивке по типам запросов (запросов в секунду) - для основных запросов**  
    ...
    ---------| ----------|----------
    регистрация | 0,023 | 0,11 
    авторизация | 1 040 | 520 
    просмотр входящих писем | 208 333,3 | 104 166,6 
    просмотр исходящих писем | 41 666,6 | 20 833,3 
    чтение письма | 208 333,3  | 104 166,6 
    отправка письма | 208 333,3 | 104 166,6 
    удаление письма | 62 500 | 31 250 
    добавление отправителя в спам | 1 040 | 520 
    редактирование личного профиля | 104 | 52 
...
* **Пиковое потребление в теченнии суток (в Гбит/с) - разбивка по существенным типам трафика (для выбора сетевых интерфейсов)**  
    ...
    ---------| ----------|----------
    просмотр входящих писем | 208 333,3 | 208 333,3 * 50 * 240 = **20 Гбит/с**
    просмотр исходящих писем | 41 666,6 | 41 666,6 * 50 * 240 = **4 Гбит/с**
    чтение письма | 208 333,3  | 208 333,3 * 75 Кб = **125 Гбит/с**
    отправка письма | 208 333,3 | 208 333,3 * 75 Кб = **125 Гбит/с**
...
* **Суммарный суточный (Гбайт/сутки) - разбивка по существенным типам трафика (опционально, для подсчета стоимости хостинга)**  
    ...
    ---------|----------
    просмотр входящих писем |  20 Гбит/с * 20 * 60 * 60 * 24 = **216 000 Гбайт/сутки**
    просмотр исходящих писем | 4 Гбит/с * 20 * 60 * 60 * 24 =  **43 200 Гбайт/сутки**
    чтение письма | 12,5 Гбит/с * 20 * 60 * 60 * 24 = **135 000 Гбайт/сутки**
    отправка письма | 12,5 Гбит/с * 20 * 60 * 60 * 24 = **135 000 Гбайт/сутки**
